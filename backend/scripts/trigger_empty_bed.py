#!/usr/bin/env python3
"""Demo 用：偵測到鏡頭前沒有人，才觸發 101 床「空床」（離床超過半小時）。

用法：
    python backend/scripts/trigger_empty_bed.py [server_url] [--debug]

連到 101 床即時姿勢資料（/ws/room/101，跟真的板子共用同一份 in_camera/
current_posture），等偵測到「鏡頭前沒有人」（in_camera=false）才呼叫
POST /api/beds/101/demo-event。Ctrl+C 可以中途取消。預設 server_url 是
http://localhost:8000。
...
def clear_stale_event(server_url: str) -> None:
    """觸發前先 resolve 掉 101 床任何還 active 的空床事件，讓這次一定是全新的一筆。"""
    try:
...
def wait_for_empty(ws_url: str, debug: bool) -> None:
    ...
    print(f"等待偵測：{BED_ID} 床鏡頭前沒有人...{suffix}")
